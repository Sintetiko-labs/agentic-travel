// Command vueling is an unofficial, agent-friendly CLI for Vueling.
package main
...
func usage() {
	fmt.Fprintf(os.Stderr, `vueling — unofficial CLI for Vueling

USAGE:
  vueling search [--json] --from ORIGIN --to DEST --depart DATE [--return DATE]
  vueling read [--json] <id|url>
  vueling brands
  vueling session chrome [--wait] [--port N]
  vueling session sync
  vueling session doctor [--json]
  vueling network doctor [--json]
