...
import (
	...
	"github.com/fbelchi/hipotels-cli/internal/client"
	"github.com/fbelchi/travelkit/session"
)

func cmdSession(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: hipotels session <chrome|sync|doctor>")
	}
	switch args[0] {
	case "chrome":
...
func runSessionChrome(args []string, syncOnly bool) error {
	...
	cl := client.New("")
	startURL := "https://www.hipotels.com/es"
	if !syncOnly {
	...
	if result.Ready {
		...
		fmt.Fprintln(os.Stderr, "Warning: WAF cookies incomplete — re-run: hipotels session chrome --wait --timeout 3m")
	}
	return nil
}
...
func runSessionDoctor(args []string) error {
	...
	res := session.Doctor(session.DoctorOptions{
		...
		BaseURL:     client.BaseURL,
		Cookie:      cl.Cookie,
		ProbeURL:    "https://www.hipotels.com/",
		ProbeMethod: "GET",
	})
	if cf.jsonOut {
		return emitJSON(res)
