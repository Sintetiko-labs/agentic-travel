...
func (c *Client) Search(query string, page, pageSize int) (*HotelSearchResult, error) {
	...
	path := "/en-us/hotels/destination/" + tkhotel.RadissonDestinationSlug(query)
	html, err := c.FetchHTML(c.BaseURL + path)
	if err != nil {
		if he, ok := err.(*tkbase.HTTPError); ok && akamai.IsDenied(he.Status, he.Body) {
			return nil, fmt.Errorf("akamai blocked — %s", akamai.NeedsSessionHint("radisson"))
		}
	...
	return tkhotel.LDToResultParent(rows, query, page, pageSize, c.Brand, "radisson", c.BaseURL, "destination"), nil
}
