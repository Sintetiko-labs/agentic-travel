...
func (c *Client) Search(query string, page, pageSize int) (*HotelSearchResult, error) {
	...
	path := "/en/city/" + tkhotel.NumaCitySlug(query)
	html, err := c.FetchHTML(c.BaseURL + path)
	if err != nil {
		if he, ok := err.(*tkbase.HTTPError); ok && akamai.IsDenied(he.Status, he.Body) {
			return nil, fmt.Errorf("akamai blocked — %s", akamai.NeedsSessionHint("numa"))
		}
	...
	rows := parse.HotelsFromNumaCity(html, c.BaseURL)
	...
	return tkhotel.LDToResult(rows, query, page, pageSize, b, c.BaseURL, "city"), nil
}
