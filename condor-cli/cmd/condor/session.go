...
import (
	...
	"github.com/fbelchi/condor-cli/internal/client"
	"github.com/fbelchi/travelkit/session"
)

func cmdSession(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: condor session <chrome|sync|doctor>")
	}
	switch args[0] {
	case "chrome":
...
func runSessionChrome(args []string, syncOnly bool) error {
	...
	cl := client.New("")
	startURL := "https://www.condor.com/es"
	if !syncOnly {
	...
	if result.Ready {
		...
		fmt.Fprintln(os.Stderr, "Warning: WAF cookies incomplete — re-run: condor session chrome --wait --timeout 3m")
	}
	return nil
}
