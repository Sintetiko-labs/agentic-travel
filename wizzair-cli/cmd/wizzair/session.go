...
func cmdSession(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: wizzair session <chrome|sync|doctor>")
	}
	switch args[0] {
	case "chrome":
...
func runSessionChrome(args []string, syncOnly bool) error {
	...
	if result.Ready {
		...
		fmt.Fprintln(os.Stderr, "Warning: WAF cookies incomplete — re-run: wizzair session chrome --wait --timeout 3m")
	}
	return nil
}
