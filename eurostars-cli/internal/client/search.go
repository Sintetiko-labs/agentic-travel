...
func (c *Client) Search(query string, page, pageSize int) (*HotelSearchResult, error) {
	...
	rows := parse.HotelsFromEurostarsEmbedded(html, c.BaseURL)
	filtered := filterHotelLD(rows, query)
	if len(filtered) == 0 {
		return nil, fmt.Errorf("search %q: no hotels — try a destination like Madrid or Barcelona", query)
	}
	return ldToResult(filtered, query, page, pageSize, brandOrDefault(c.Brand, "Eurostars Hotel Company"), c.BaseURL, "embedded-json"), nil
}
