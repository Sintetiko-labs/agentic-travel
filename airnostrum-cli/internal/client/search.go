...
func (c *Client) Search(origin, dest, depart, ret string, page, pageSize int) (*FlightSearchResult, error) {
	...
	q := fmt.Sprintf("?market=ES&language=es&origin=%s&destination=%s&departureDate=%s&adults=1&operatingCarrier=YW", origin, dest, depart)
	c.Throttle()
	...
	if akamai.IsWAFBlocked(resp.StatusCode, string(raw)) {
		return nil, fmt.Errorf("akamai blocked — %s", akamai.NeedsSessionHint("airnostrum"))
	}
	...
	for _, f := range items {
		flights = append(flights, FlightHit{ID: f.ID, Airline: "Air Nostrum", FlightNumber: f.FlightNumber, Origin: f.Origin, Destination: f.Destination, Depart: f.Departure, Arrive: f.Arrival, Price: fmt.Sprintf("%.2f", f.Price), Currency: tkbase.FirstNonEmpty(f.Currency, "EUR"), BookingURL: "https://www.iberia.com/es/vuelos/"})
	}
	total := len(flights)
