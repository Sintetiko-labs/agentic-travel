...
func cmdSession(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: iberostar session <chrome|sync|doctor>")
	}
	switch args[0] {
	case "chrome":
...
func runSessionChrome(args []string, syncOnly bool) error {
	...
	cl := client.New("")
	startURL := "https://www.iberostar.com/es/hoteles"
	if !syncOnly {
		fmt.Fprintln(os.Stderr, "Capturing session from headed Chrome…")
		fmt.Fprintf(os.Stderr, "  Save to: %s\n", cl.CookiesFilePath())
		fmt.Fprintf(os.Stderr, "  URL: %s\n", startURL)
		if doWait {
			fmt.Fprintln(os.Stderr, "  Waiting for site cookies (_session_id) — browse the hotel listing if needed")
		}
	}
	...
	if result.Ready {
		...
		fmt.Fprintln(os.Stderr, "Warning: WAF cookies incomplete — re-run: iberostar session chrome --wait --timeout 3m")
	}
	return nil
}
