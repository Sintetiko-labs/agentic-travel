...
func (c *Client) Search(query string, page, pageSize int) (*HotelSearchResult, error) {
	...
	rows := parse.HotelsFromVincciLinks(html, c.BaseURL)
	...
	return ldToResult(filtered, query, page, pageSize, brandOrDefault(c.Brand, "Vincci Hoteles"), c.BaseURL, "html-links"), nil
}
