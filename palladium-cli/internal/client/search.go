...
// Search finds hotels via data-hotel-name cards on Palladium listing pages.
func (c *Client) Search(query string, page, pageSize int) (*HotelSearchResult, error) {
	...
	paths := []string{hotelsListingPath, "/es/hoteles/espana", "/es/hoteles/mexico"}
	...
	for _, p := range paths {
		...
		rows = append(rows, parse.HotelsFromPalladiumCards(html, c.BaseURL)...)
	}
	...
	return palladiumToResult(filtered, query, page, pageSize, c.Brand), nil
}

func palladiumToResult(rows []parse.HotelLD, query string, page, pageSize int, brand string) *HotelSearchResult {
	total := len(rows)
	...
	return &HotelSearchResult{
		...
		HasNext: total > page*pageSize, Hotels: hits, Brand: brand, Source: "html-cards",
	}
}
