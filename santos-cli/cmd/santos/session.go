...
func cmdSession(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: santos session <chrome|sync|doctor>")
	}
	switch args[0] {
	case "chrome":
...
func runSessionChrome(args []string, syncOnly bool) error {
	...
	cl := client.New("")
	startURL := "https://www.hoteles-santos.com/es"
	if !syncOnly {
	...
	if result.Ready {
		...
		fmt.Fprintln(os.Stderr, "Warning: WAF cookies incomplete — re-run: santos session chrome --wait --timeout 3m")
	}
	return nil
}
...
func runSessionDoctor(args []string) error {
	...
	res := session.Doctor(session.DoctorOptions{
		...
		BaseURL:     client.BaseURL,
		Cookie:      cl.Cookie,
		ProbeURL:    "https://www.hoteles-santos.com/",
		ProbeMethod: "GET",
	})
	if cf.jsonOut {
		return emitJSON(res)
