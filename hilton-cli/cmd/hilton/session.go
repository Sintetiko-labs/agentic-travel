...
func cmdSession(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: hilton session <chrome|sync|doctor>")
	}
	switch args[0] {
	case "chrome":
...
func runSessionChrome(args []string, syncOnly bool) error {
	...
	cl := client.New("")
	startURL := "https://www.hilton.com/en/locations/united-kingdom/london/"
	if !syncOnly {
	...
	if result.Ready {
		...
		fmt.Fprintln(os.Stderr, "Warning: WAF cookies incomplete — re-run: hilton session chrome --wait --timeout 3m")
	}
	return nil
}
