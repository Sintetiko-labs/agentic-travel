...
func (c *Client) Search(query string, page, pageSize int) (*HotelSearchResult, error) {
	...
	rows := parse.HotelsFromSilkenCards(html, c.BaseURL)
	filtered := filterHotelLD(rows, query)
	if len(filtered) == 0 {
		return nil, fmt.Errorf("search %q: no hotels — Silken has no Madrid properties; try Barcelona or Bilbao", query)
	}
	return ldToResult(filtered, query, page, pageSize, brandOrDefault(c.Brand, "Silken Hoteles"), c.BaseURL, "html-cards"), nil
}
