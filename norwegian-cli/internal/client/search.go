...
func (c *Client) Search(origin, dest, depart, ret string, page, pageSize int) (*FlightSearchResult, error) {
	...
	if _, err := c.FetchHTML(ref); err != nil {
		if he, ok := err.(*tkbase.HTTPError); ok && akamai.IsDenied(he.Status, he.Body) {
			return nil, fmt.Errorf("akamai blocked — %s", akamai.NeedsSessionHint("norwegian"))
		}
	}
	q := url.Values{}
	q.Set("Origin", origin)
	...
	body, status, err := c.apiGET(c.BaseURL+"/resourceipr/api/airportpaircalendarflexibility?"+q.Encode(), ref)
	if err != nil {
		return nil, fmt.Errorf("search %s→%s: %w", origin, dest, err)
	}
	if err := checkAPI(status, body, "norwegian"); err != nil {
		return nil, fmt.Errorf("search %s→%s: %w", origin, dest, err)
	}
	brand := c.Brand
	if brand == "" {
	...
	return paginateFlightResult(origin, dest, depart, ret, page, pageSize, flights, brand, "airportpaircalendarflexibility"), nil
}
